...
- `order.http` - 订单相关接口测试（预期接口 + 数据库查询）
- `instance.http` - 实例相关接口测试（预期接口 + 数据库查询）
...
1. 确保已有订单数据（通过秒杀或支付创建）
2. 使用 `order.http` 中的 SQL 查询订单
3. 验证订单状态、来源、金额等信息

### 实例查询测试

1. 确保订单已创建实例
2. 使用 `instance.http` 中的 SQL 查询实例日志
3. 验证实例创建状态
...
1. 部分接口（订单、实例查询）尚未在 proto 中定义，标记为注释状态
2. 秒杀功能主要通过 Redis Stream 消费，不直接暴露 HTTP 接口
3. 实例管理由资源域负责，商品域只负责生成 ID 和发送 MQ 消息
